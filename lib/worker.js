...
function renderFft(ctx) {
    ...
    const imageData = { data: new Uint8ClampedArray(4 * width * height) };
    ...
    for (let x = 0; x < width; x++) {
        ...
        for (let k = 0; k < n; k++) {
            ...
            real[k] = window[k] * sampleView.sampleI(pos);
            imag[k] = window[k] * sampleView.sampleQ(pos);
        }
